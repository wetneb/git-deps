...
class DependencyDetector(object):
    ...
    def find_dependencies_with_parent(self, dependent, parent):
        ...
        self.register_new_dependent(dependent, dependent.hex)
        diff = self.repo.diff(parent, dependent,
                              context_lines=self.options.context_lines)

        def generate_blame_hunks():
            for patch in diff:
                path = patch.delta.old_file.path
                self.logger.info("      Examining hunks in %s" % path)
                for hunk in patch.hunks:
                    # If the following check fails, this is probably because
                    # dependent added a new directory which was not previously
                    # in the parent.
                    if self.tree_lookup(path, parent):
                        ...
                        for blame_hunk in blame:
                            yield {
                                'path': path,
                                'blame_hunk': blame_hunk
                            }

        tasks = generate_blame_hunks()

        def get_commit_time(blame_hunk_with_path):
            committer = blame_hunk_with_path['blame_hunk'].orig_committer
            if committer:
                return - committer.time
            else:
                return 0

        tasks = sorted(tasks, key=get_commit_time)

        for blame_hunk_with_path in tasks:
            self.process_blame_hunk(dependent, parent, blame_hunk_with_path['path'],
                blame_hunk_with_path['blame_hunk'])

    def process_blame_hunk(self, dependent, parent,
                          path, blame_hunk):
        dependent_sha1 = dependent.hex
